# ['Ee-Cs' GitHub Pages](https://Ee-Cs.github.io)
[Portfolio](https://github.com/k1729p/Portfolio)

---

<details>
<summary>⭕ Top 20 AI Chat Interfaces</summary>

| Name of LLM | Creator| Main Chat Link |
| --- | --- | --- |
| 🟡**ChatGPT** | OpenAI | [chatgpt.com](https://chatgpt.com/) |
| 🔴**Claude** | Anthropic | [claude.ai](https://claude.ai/) |
| 🔴**Gemini** | Google | [gemini.google.com](https://gemini.google.com/) |
| 🟡**Copilot** | Microsoft | [copilot.microsoft.com](https://copilot.microsoft.com/) |
| 🟡**DeepSeek** | DeepSeek | [chat.deepseek.com](https://chat.deepseek.com/) |
| 🟠**Le Chat** | Mistral AI | [chat.mistral.ai](https://chat.mistral.ai/) |
| 🟠**Grok** | xAI | [grok.com](https://grok.com/) |
| **Llama** | Meta | [llama.com](https://www.llama.com/) |
| **Kimi** | Moonshot AI | [kimi.moonshot.cn](https://kimi.moonshot.cn/) |
| **Qwen** | Alibaba Cloud | [chat.qwen.ai](https://chat.qwen.ai/) |
| 🔴**Perplexity** | Perplexity | [perplexity.ai](https://www.perplexity.ai/) |
| **Pi** | Inflection AI | [pi.ai](https://pi.ai/) |
| **Character.ai** | Character.ai | [character.ai](https://character.ai/) |
| **HuggingChat** | Hugging Face | [huggingface.co/chat](https://huggingface.co/chat/) |
| **Poe** | Quora | [poe.com](https://poe.com/) |
| **OpenRouter** | OpenRouter | [openrouter.ai/chat](https://openrouter.ai/chat/) |
| **Gemma** | Google | [ai.google.dev/gemma](https://ai.google.dev/gemma/) |
| **Command R** | Cohere | [cohere.com/chat](https://cohere.com/chat/) |
| **Jamba** | AI21 Labs | [ai21.com](https://www.ai21.com/) |
| 🟡**Gordon** | Docker | [docs.docker.com/ai/gordon](https://docs.docker.com/ai/gordon/) |

---
</details>

<details>
<summary>⭕ Markdown Files</summary>

| |
| --- |
| [**emojis.md**](../files/emojis.md) |
| [**fill-and-stroke-colors.md**](../files/fill-and-stroke-colors.md) |
| [**headings.md**](../files/headings.md) |
| [**lists-and-tables.md**](../files/lists-and-tables.md) |
| [**fenced-code-blocks.md**](../files/fenced-code-blocks.md) |
| [**flowchart-diagrams.md**](../files/flowchart-diagrams.md) |
| [**sequence-diagrams.md**](../files/sequence-diagrams.md) |
| [**class-diagrams.md**](../files/class-diagrams.md) |
| [**state-diagrams.md**](../files/state-diagrams.md) |
| [**radar-diagrams.md**](../files/radar-diagrams.md) |
| [**geojson.md**](../files/geojson.md) |
           
---
</details>

<details>
<summary>⭕ HTML Pages</summary>

| |
| --- |
| [**scripting.html**](https://ee-cs.github.io/pages/scripting.html) |
| [**es6-inheritance.html**](https://ee-cs.github.io/pages/es6-inheritance.html) |
| [**tables-and-emojis.html**](https://ee-cs.github.io/pages/tables-and-emojis.html) |
| [**font-families.html**](https://ee-cs.github.io/pages/font-families.html) |
| [**tree.html**](https://ee-cs.github.io/pages/tree.html) |
| [**small-icons.html**](https://ee-cs.github.io/pages/small-icons/small-icons.html) |
           
---
</details>

---
<!-- This content will not appear in the rendered Markdown -->
<a name="top"></a>
### GitHub Docs
- [Basic writing and formatting syntax](https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax)
- [Working with advanced formatting](https://docs.github.com/en/get-started/writing-on-github/working-with-advanced-formatting)

---


[Back to the top of the page](#top)